...
def best_outline(small_img, large_img, EoLobjectID, param_dir = None, composite_file_dir = None, butterfly_with_contour_file_dir = None):
    ...
    # for windows platforms, this workaround resizes large images so tht grabcut will work consistently
    # see   http://code.opencv.org/issues/3468 :  OpenCV bug #3468
    if min(H,W) > int(2000):
        rescale_p = 1600/min(H,W) #rescales image so smaller dimension is 1600 px
        large_img = cv2.resize(large_img, None, fx=rescale_p,fy=rescale_p, interpolation = cv2.INTER_AREA)
        H, W = large_img.shape[:2]
...
if __name__ == "__main__":
    contour_dir = None #"contours" #set to None unless you want to output params to model probability that a contour outline is a butterfly.
    ...
    images = 'RawDataSubset.csv' #If this is a folder, it should contain ID_580_360.jpg files together with the full-sized ID.xxx files. If it a filename, should be a csv file of ID,URL
    #images = 'images'
